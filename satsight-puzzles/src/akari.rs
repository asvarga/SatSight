...
....1..",
    ...
    /// A 10×10 puzzle with a **unique** solution that pure logic alone can't
    /// crack — the "hard" demo board. Unlike [`sample`](Akari::sample), whose
    /// number-heavy 7×7 propagation solves outright, this one is sparser:
    /// propagation and probing fill about two-thirds of it and
    /// then stall, so the rest genuinely needs search — exactly the case the Full
    /// solve / Step / Backbone views exist to show. It was generated by numbering
    /// a solved layout and then dropping every clue whose removal kept the board
    /// unique, so it is minimal as well as hard.
    #[must_use]
...
...0..1...",
        ...
        .expect("the built-in hard sample is a valid board")
    }
}
...
mod tests {
    ...
    /// Count white cells decided (a lamp or a proven empty) in a projection.
    fn decided_white(a: &Akari, grid: &Grid<AkariCell>) -> usize {
        let mut n = 0;
        for r in 0..a.rows() {
            for c in 0..a.cols() {
                if matches!(grid.get(r, c), AkariCell::White { lamp: Some(_), .. }) {
                    n += 1;
                }
            }
        }
    ...
    fn white_total(a: &Akari) -> usize {
        (0..a.rows())
            .flat_map(|r| (0..a.cols()).map(move |c| (r, c)))
            .filter(|&(r, c)| a.is_white(r, c))
            .count()
    }

    fn is_unique(a: &Akari) -> bool {
        let bb = backbone(a);
        bb.satisfiable && decided_white(a, &a.project_deductions(&bb)) == white_total(a)
    }
    ...
    #[test]
    fn hard_sample_is_unique_but_needs_search() {
        // The hard board must (a) solve to a legal lighting, (b) be unique — its
        // backbone fills every white cell — and (c) actually be harder than the
        // sample: pure logic (propagation + probing) must leave some cell undecided,
        // so search is genuinely required. That last check is what makes it the
        // "hard" demo rather than just a bigger easy one.
        let a = Akari::hard_sample();
        let grid = solve(&a).expect("the hard sample is solvable");
        assert!(
            obeys_rules(&a, &grid, true),
            "the hard solve obeys every rule"
        );

        assert!(is_unique(&a), "the hard sample has a unique solution");

        let logic = decided_white(&a, &a.project_deductions(&deduce(&a)));
        assert!(
            logic < white_total(&a),
            "logic alone must not finish the hard board — search is the point"
        );
    }
